...
class ScreenHandle(pygame.Surface):
    ...
    def _draw_next(self, canvas):
        if self.successor is not None:
            canvas.blit(self.successor, self.next_coord)
            self.successor.draw(canvas)

    def draw(self, canvas):
        self._draw_next(canvas)

    def _pass_engine(self, engine):
        if self.successor is not None:
            self.engine = engine
            self.successor.connect_engine(engine)

    def connect_engine(self, engine):
        self._pass_engine(engine)
...
class GameSurface(ScreenHandle):
    ...
    def calculate_start_coord(self, minx, miny):
        minx = max(self.engine.hero.position[0] - 6, minx)
        miny = max(self.engine.hero.position[1] - 6, miny)

        return minx, miny

    def draw_map(self):
        min_x, min_y = self.calculate_start_coord(0, 0)
        # min_x, min_y = (0, 0)
        ...
        for obj in self.engine.objects:
            self.blit(obj.sprite[0],
                      ((obj.position[0] - min_x) * self.engine.sprite_size,
                       (obj.position[
                            1] - min_y) * self.engine.sprite_size))
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        pygame.draw.rect(self, colors[
            "red"], (50, 30,
                     200 * self.engine.hero.hp / self.engine.hero.max_hp, 30))
        pygame.draw.rect(self, colors["green"], (50, 70,
                                                 200 * self.engine.hero.exp / (
                                                         100 * (2 ** (
                                                         self.engine.hero.level - 1))),
                                                 30))
        ...
        self.blit(
            font.render(f'{self.engine.level} floor', True, colors["black"]),
            (10, 0))
        ...
        self.blit(
            font.render(f'{self.engine.hero.hp}/{self.engine.hero.max_hp}',
                        True, colors["black"]),
            (60, 30))
        ...
        self.blit(
            font.render(f'{self.engine.hero.level}', True, colors["black"]),
            (360, 30))
        self.blit(
            font.render(f'{self.engine.hero.gold}', True, colors["black"]),
            (360, 70))
        ...
        self.blit(font.render(f'SCORE', True, colors["black"]),
                  (550, 30))
        self.blit(
            font.render(f'{self.engine.score:.4f}', True, colors["black"]),
            (550, 70))
...
class HelpWindow(ScreenHandle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.data.append(["2", "Zoom +"])
        self.data.append(["1", "Zoom -"])
