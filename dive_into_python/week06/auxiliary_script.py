...
def run(host, port):
    ...
    try:
        ...
        print("Неверная команда, отправленная серверу, "
              "должна возвращать ошибку протокола")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Ошибка вызова метода get с ключом, который еще "
              f"не был добавлен: {err.__class__} {err}")
        sys.exit(1)
